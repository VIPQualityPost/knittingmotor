...
{
  if (currentConfig.cfg.buttonsInitialized && !buttonsLoaded)
  {
    loadButtonValues(currentConfig.cfg.buttonValues);
    buttonsLoaded = true;
  }
  else if (!buttonsLoaded)
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      currentConfig.cfg.buttonValues[i] = buttonValues[i];
    }
    currentConfig.cfg.buttonsInitialized = true;
    currentConfig.save();
    loadButtonValues(currentConfig.cfg.buttonValues);
    buttonsLoaded = true;
  }
...
void loop()
{
  ...
  else if (btn) // if any button pressed, but no beep enabled
  {
    // do nothing
    DBG_PRINTLN(F("Button pressed, but no beep enabled"));
  }

  // footswitch installed?
  if (currentAppMode == APP_PRE_CHECK && 0 && digitalRead(footswitchPin) == signalLevel)
  {
