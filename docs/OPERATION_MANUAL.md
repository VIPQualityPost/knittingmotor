...
After powering on the control box, you will have to reset (aka homing) the knitting carriage once.
...
If, for whatever reason, you encounter any problems (some unknown error, menu not responding, whatever) and want to start over, simply hit the `RESET`button. The button will restart the control box and you are good to go. You will not loose the row count, if your box is in `auto` mode.(I.e. menu not responding *can* happen after one of the safety sensors was engaged.)

## 3. The menu system and important parameters
...
- Direction arrow (default: `on`)<br>
  ...
  *Possible symbols:*<br>
  ...
  Example:<br>
  - If the symbol combination shown is `[C->]` it means, that the last row was knitted to the right between boundaries and continuous knitting is on.
  - If the symbol shown is only `<-` it means, that the last row was knitted to the left.
